...
playstore[['Category']] =\
playstore[['Category']].astype('category')
...
playstore[['Installs']] =\
playstore[['Installs']].astype('int64')
...
playstore[['Price']] =\
playstore[['Price']].astype('float64')
...
def index():
    ...
    stats = {
        ...
        # Agregasi Anda dinilai benar jika hasilnya sama dengan tabel yang terlampir pada file ini
         'rev_table' : playstore [['Category', 'Reviews', 'Rating','App']].groupby (['Category','App']).agg({
         'Reviews': 'sum',
         'Rating': 'mean',
    ...
    #Category aplikasi mana yang paling banyak dipakai berdasarkan ratingnya?
    # playstore.groupby('Category').agg({'Rating':'sum'}).sort_values(by='Rating',ascending=False).head(5 
    playstore.groupby('Category').agg({'Rating':'sum'}).sort_values(by='Rating',ascending=False).head(5).plot(kind='bar')
    figfile = BytesIO()
    plt.savefig(figfile, format='png')
    figfile.seek(0)
    figdata_png = base64.b64encode(figfile.getvalue())
